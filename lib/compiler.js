...
module.exports = function() {
	...
	var Exception = __ks__.Exception, IOException = __ks__.IOException, NotImplementedException = __ks__.NotImplementedException, NotSupportedException = __ks__.NotSupportedException, ReferenceException = __ks__.ReferenceException, SyntaxException = __ks__.SyntaxException, TypeException = __ks__.TypeException, CodeFragment = __ks__.CodeFragment, FragmentBuilder = __ks__.FragmentBuilder, ControlBuilder = __ks__.ControlBuilder, BlockBuilder = __ks__.BlockBuilder, ExpressionBuilder = __ks__.ExpressionBuilder, LineBuilder = __ks__.LineBuilder, ObjectBuilder = __ks__.ObjectBuilder, ArrayBuilder = __ks__.ArrayBuilder, Module = __ks__.Module, Compiler = __ks__.Compiler, compileFile = __ks__.compileFile, getBinaryPath = __ks__.getBinaryPath, getHashPath = __ks__.getHashPath, getMetadataPath = __ks__.getMetadataPath, isUpToDate = __ks__.isUpToDate, extensions = __ks__.extensions;
	...
	function $constructor(members, data, coverage, coverageName, file, node) {
		...
		for(var __ks_0 = 0, __ks_1 = data.parameters.length, parameter; __ks_0 < __ks_1; ++__ks_0) {
			...
			if(fields[parameter.name.name] !== true) {
				__ks_arguments_1.push(parameter.name);
			}
		}
		data.body = {
			kind: NodeKind.Block,
	...
	$expressions[NodeKind.ExportAlias] = function(data, coverage, coverageName, file, node) {
		if(arguments.length < 5) {
			throw new SyntaxError("wrong number of arguments (" + arguments.length + " for 5)");
		}
		if(data === void 0 || data === null) {
	...
	var CoverageModule = Helper.class({
		...
		__ks_func_parse_0: function(data, file) {
			...
			var reducePath = this.reducePath;
			var coverage;
