...
module.exports = function() {
	...
	function $block(init, data, coverage, coverageName, location, file, node) {
		...
		if(data.kind.valueOf() === NodeKind.Block.value) {
	...
	function $body(data, location) {
		...
		else if(data.kind.valueOf() === NodeKind.Block.value) {
			return data;
		}
		else {
			return $location((() => {
	...
	function $constructor(members, data, coverage, coverageName, file, node) {
		...
		for(let __ks_0 = 0, __ks_1 = members.length, member; __ks_0 < __ks_1; ++__ks_0) {
			...
			if(member.kind.valueOf() === NodeKind.FieldDeclaration.value) {
	...
	const $expressions = (() => {
		...
		d[NodeKind.BinaryExpression] = function(data, coverage, coverageName, file, node) {
			...
			if((data.operator.kind.valueOf() === BinaryOperatorKind.And.value) || (data.operator.kind.valueOf() === BinaryOperatorKind.Or.value)) {
			...
			else if(data.operator.kind.valueOf() === BinaryOperatorKind.Assignment.value) {
		...
		d[NodeKind.CurryExpression] = function(data, coverage, coverageName, file, node) {
			...
			data.callee = $compile.expression(data.callee, coverage, coverageName, file, node);
			if(data.scope.kind.valueOf() === ScopeKind.Argument.value) {
	...
	function $if(condition, whenTrue, coverage, coverageName, file, node) {
		...
		if(whenTrue.kind.valueOf() === NodeKind.Block.value) {
	...
	function $location() {
		if(arguments.length === 2 && Type.isDictionary(arguments[1])) {
			let __ks_i = -1;
	...
	function $sequence(init, data, coverage, coverageName, file, node) {
		...
		if(data.kind.valueOf() === NodeKind.SequenceExpression.value) {
	...
	class CoverageModule extends Module {
		...
		__ks_cons_0(data, coverageName, compiler, file) {
			...
			else if(!Type.isInstance(compiler, Compiler)) {
