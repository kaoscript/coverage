...
module.exports = function() {
	...
	$statements[NodeKind.ExportDeclaration] = function(data, coverage, coverageName, file, node) {
		...
		data.declarations = Helper.mapArray(data.declarations, function(declaration) {
			return Type.isValue($statements[declaration.kind]) ? $statements[declaration.kind](declaration, coverage, coverageName, file, node) : $expressions[declaration.kind](declaration, coverage, coverageName, file, node);
		});
		return data;
	};
	...
	var CoverageCompiler = Helper.class({
		...
		__ks_func_compile_0: function(data) {
			...
			if(this._instrument) {
				var coverage = this._module.instrument(this._coverageName, this._file);
				...
				var line = header.newLine();
				line.code("if(!" + this._coverageName + "[\"" + this._file + "\"]) {\n");
				line.code("\t" + this._coverageName + "[\"" + this._file + "\"] = {");
				line.code("\"path\":\"" + this._file + "\",");
				line.code("\"s\":{");
				...
				line.code("}");
				line.done();
				this._fragments = header.toArray().concat(this._module.toFragments());
			}
			else {
				this._module.analyse();
				this._module.fuse();
	...
	Helper.newInstanceMethod({
		class: Module,
		name: "instrument",
		function: function(coverageName, file) {
			if(coverageName === undefined || coverageName === null) {
				throw new Error("Missing parameter 'coverageName'");
			}
			if(file === undefined || file === null) {
			...
			this._data.body = $compile.statements(this._data.body, coverage, coverageName, file, this);
			this._data.body.unshift({
