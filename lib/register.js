...
function excludeFile(filename) {
	...
	for(let __ks_0 = 0, __ks_1 = excludes.length, exclude; __ks_0 < __ks_1; ++__ks_0) {
		...
		if(filename.startsWith(exclude) === true) {
			return true;
		}
	}
	return false;
...
function loadOriginalFile(module, filename) {
	...
	if((fs.isFile(getBinaryPath(filename, target)) === true) && (fs.isFile(getHashPath(filename, target)) === true) && (isUpToDate(filename, target, source) === true)) {
...
function findGlobalVariable() {
	const coverageVar = Helper.concatString("$$cov_", Date.now(), "$$");
	if(Type.isValue(global[coverageVar])) {
		return coverageVar;
	}
	else {
		const coverageVars = Helper.mapDictionary(global, function(key) {
			return key;
		}, function(key) {
...
const target = Operator.gte(parseInt(/^v(\d+)\./.exec(process.version)[1]), 6) ? "ecma-v6" : "ecma-v5";
